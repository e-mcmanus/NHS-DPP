...
# Type 2 Diabetes
# Death
#SA6: Using data from Keng et al. (2022) for the distribution of type 2 diabetes
######################################## 
...
#Cohort is made up of 6 age categories / model runs
...
#Transition probabilities (later replaced with PSA values)
...
#change in transition probability for DPP (later replaced with PSA values)
effect <- 0.8
...
# same for both treatments - no cost of death applied
...
# Sum costs and QALYs for each cycle at a time for each treatment  
...
# #change in transition probability for DPP
effect <- 0.8
...
p_matrix_cycle <- function(p_matrix, age, cycle,
  ...
  tpProgT2D_lookup <-
    c("[30,39]" = rbeta(1,27.12014,972.8799),
      "[40,49]" = rbeta(1,34.49617,965.5038),
      "[50,59]" = rbeta(1,30.72741,969.2726),
      "[60,69]" = rbeta(1,25.45268,974.5473),
      "[70,79]" = rbeta(1,20.27103,979.729),
      "[80,120]" = rbeta(1,14.16019,985.84731))
...
state_q_matrix_cycle <- function(state_q_matrix, age)
{
  ...
  #uT2D
  uT2D_lookup <-
    c("[30,39]" = rbeta(1, 32561.2, 9671.2),
      "[40,49]" = rbeta(1, 32561.2, 9671.2),
      "[50,59]" = rbeta(1, 32561.2, 9671.2),
      "[60,69]" = rbeta(1, 32561.2, 9671.2),
      "[70,79]" = rbeta(1, 32561.2, 9671.2),
      "[80,120]" = rbeta(1, 32561.2, 9671.2))
...
for(n in 1:n_trials) {
  ...
  for (i in 1:n_treatments) {
    ...
    total_costs[i] <- sum(cycle_costs[treatment = i, -1])
    total_QALYs[i] <- sum(cycle_qalys[treatment = i, -1])
    #if we want to include FIRST 'STARTING' CYCLE total_costs[i] <- rowSums(cycle_costs)[i]
    #if we want to include FIRST 'STARTING' CYCLE  total_QALYs[i] <- rowSums(cycle_QALYs)[i]
  }
  #adding in costs of DPP participation
  total_costs["with DPP"] <- total_costs["with DPP"] + dppcost_sum
  #adding in benefits from DPP participation
...
for(n in 1:n_trials) {
  ...
  #adding in costs of DPP participation
  total_costs["with DPP"] <- total_costs["with DPP"] + dppcost_sum
  # adding in benefits of DPP participation
  total_QALYs["with DPP"] <- total_QALYs["with DPP"] + 0.5*utility_sum
...
#can't work out how to put it in a loop!
...
#manually changed length to allow for additional CEAC estimates so there isnt a 'kink' in the graph
per_ce<-c(rep(0,length(67)))
...
#find out range of x variable to ensure plot is correct
range(PSA_results$qaly_difpp)
#-0.01339027  0.07249291
