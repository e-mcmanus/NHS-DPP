...
# Type 2 Diabetes
# Death
#SA1: Effect maintained for 3 years (HR 0.800), and then reduced to (HR: 0.883) for the following 7 years, then nothing
########################################
...
cDeath <- 0
cDPP <- 0
#rgamma(1,16.73964,scale=8.469118)
...
#only cost associated with death – no cost applied in this model
trans_c_matrix <-
...
p_matrix_cycle <- function(p_matrix, age, cycle,
  ...
  effect_lookup <-
    c("(0,3]" = rlnorm(1, meanlog = -0.22314, sdlog = 0.044757),
      "(3,10]" = rlnorm(1, meanlog = -0.1244, sdlog = 0.04053),
...
for(n in 1:n_trials) {
  ...
  for (i in 1:n_treatments) {
    ...
    #cycle_trans_costs[i, ] <-
    #  (c(1,1,1,1) %*% trans[, , treatment = i]) * (1/(1 + cDr)^(1:n_cycles - 2)) #* v_wcc
    
    cycle_costs[i, ] <- cycle_state_costs[i, ] #+ cycle_trans_costs[i, ]
    
    # life expectancy
    #LE[i, ] <- c(1,1,1,0) %*% halfpop[, , treatment = i]
    
    # life-years
    #LYs[i, ] <- LE[i, ] * (1/(1 + oDr)^(-1:n_cycles - 2)) 
    
    # quality-adjusted life expectancy
    #cycle_QALE[i, ] <-
    #  state_q_matrix[treatment = i, ] %*% halfpop[, , treatment = i]
    
    # quality-adjusted life-years
    #cycle_QALYs[i, ] <- cycle_QALE[i, ] * (1/(1 + oDr)^(-1:n_cycles - 1)) #* v_wcc
...
#ne: 89, nw: 3, se: 9907, sw: 1

#renaming the variables
# names(costqalys)[names(costqalys) == "without.DPP.y"] <- "Without.DPP.qalys"
# names(costqalys)[names(costqalys) == "with.DPP.y"] <- "With.DPP.qalys"
# names(costqalys)[names(costqalys) == "without.DPP.x"] <- "Without.DPP.costs"
# names(costqalys)[names(costqalys) == "with.DPP.x"] <- "With.DPP.costs"
